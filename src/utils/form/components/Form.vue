<template>
  ...
  >
    <v-row>
      ...
      >
        <text-input
          @input="change(input.field)"
          v-if="input.field.type=='password' || input.field.type=='text'"
          :input="input.field"
          :type="input.field.type"
        />
        <select-input
          @change="change(input.field)"
          v-if="input.field.type=='select'"
          :input="input.field"
        />
        <combo-input
          @change="change(input.field)"
          v-if="input.field.type=='combo'"
          :input="input.field"
        />
        <file-input
          @change="fileChange(input.field)"
          v-if="input.field.type=='file'"
          :input="input.field"
        />
        <date-input
          @change="change(input.field)"
          v-if="input.field.type=='date'"
          :input="input.field"
        />
        <switch-input
          @change="change(input.field)"
          v-if="input.field.type=='switch'"
          :input="input.field"
        />
      </v-col>
      <v-col
...
export default Vue.extend({
  ...
  methods: {
    async submit() {
      this.validForm = await (
        this.$refs.appFrom as Vue & { validate: () => boolean }
      ).validate();
      if (this.validForm) {
        this.form.submitAction();
      }
    },
